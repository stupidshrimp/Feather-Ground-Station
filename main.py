...
class MainWindow(QMainWindow):
    ...
    def keyPressEvent(self, event):  # noqa: N802 - Qt override naming
        ...
        if event.key() in manual_mapping or event.key() in auto_speed_mapping:
            if self.throttle_mode == "Manual":
                if event.key() in manual_mapping:
                    self.target_throttle_percent = manual_mapping[event.key()]
            elif event.key() in auto_speed_mapping:
                self.set_auto_throttle_target_speed(auto_speed_mapping[event.key()])
            event.accept()
    ...
    def set_auto_throttle_target_speed(self, speed_mph: float) -> None:
        """Set the target speed sent on CH3 while auto throttle is selected."""
    ...
    def _build_control_channels(self):
        ...
        # CH3 carries manual throttle percent in Manual mode, and desired
        # airspeed in Auto Throttle mode.  The FC applies the same converter in
        # reverse before running its local throttle PID.
